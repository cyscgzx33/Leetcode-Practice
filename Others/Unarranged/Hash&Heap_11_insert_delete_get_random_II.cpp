...
public:
    ...
    bool insert(int val) {
        ...
        bool is_exist = num_index_map.count(val) > 0;
        if (!is_exist) num_index_map[val] = std::vector<int>();
    ...
    bool remove(int val) {
        ...
        num_index[cur_index] = num_index[last_index]; // overwrite the current elem as the last elem
        num_index.pop_back();
