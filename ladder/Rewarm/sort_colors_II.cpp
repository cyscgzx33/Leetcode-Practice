...
public:
	...
	void rainbowSort(std::vector<int>& nums, int nums_start, int nums_end, int colors_start, int colors_end)
	{
		...
		if (nums_start >= nums_end || colors_start >= colors_end)
			return;
		...
		int colors_mid = colors_start + (colors_end - colors_start) / 2;
		int nums_left = nums_start, nums_right = nums_end;

		while (nums_left <= nums_right)
		{
			while (nums_left <= nums_right && nums[nums_left] <= colors_mid)
				nums_left++;
			while (nums_left <= nums_right && nums[nums_right] > colors_mid)
				nums_right--;
